...
class Ezapps_Zoom_Handler
{
    ...
    function __construct() {
        ...
        self::$STORE       = array_key_exists('store', $_COOKIE) ? $_COOKIE['store'] : (array_key_exists('MAGE_RUN_CODE', $_SERVER) ? $_SERVER['MAGE_RUN_CODE'] : '');

        if ( self::$STORE == '' ) self::$STORE = 'default';
    ...
    /**
     * Implement singleton method for Zoom.
     *
     * @return Ezapps_Zoom_Handler
     */
    ...
    /**
     * Try to retrieve a previously cached file.
     *
     * @return void
     */
    public static function tryRetrieveCacheFile() {
        ...
        if ($client_matches) {
            ...
            if ($zoom_package != '' || $zoom_template != '' || $zoom_theme != '') {
                ...
                $file_to_check = self::$ZOOM_ROOT . self::DS .  self::$STORE . self::$base_uri;
        } else {
        ...
        if (count($params) > 0) {
            ...
            if (array_key_exists($page_name, $_GET)) {
                ...
                $files[] = self::paramsToFile(array_merge(array($page_name => 1), $params), $file_to_check);
        ...
        if (preg_match('/MSIE [1-6]/i', self::$AGENT))
            $gzip = false;
        else
            $gzip = true;
    ...
    /**
     * Returns a string of the provided URI params.
     *
     * @param  array $params
     * @param  string $uri
     * @return string|array
    ...
    /**
     * Perform hole punching on the configured modules.
     *
     * @param  string $buffer
     * @return string
     */
    ...
    /**
     * Start buffering with holepunch callback.
     */
    ...
    /**
     * Check the current useragent against the provided serialized regex rules.
     *
     * @param  string $regex
     * @return mixed
     */
